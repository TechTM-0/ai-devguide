...
全体の流れ：**配布準備**（初-0：開発者が1回）→ **PC 設定**（初-1〜12：使う人が PC 単位で1回）→ **プロジェクト設定**（初-13〜20：新しいプロジェクトごとに `/setup` で実行・冪等）
...
| ステップ | 担当 | 読み込み | 書き込み | すること | 次へ |
|---|---|---|---|---|---|
| 初-1 | 人間 | - | - | `git clone <ai-devguide の URL>` で任意のローカルフォルダにクローンし、そのフォルダで Claude Code を起動して `/setup` を実行する | → 初-2 |
| 初-2 | AI | `.claude/commands/setup.md` | - | `/setup` コマンドの内容（チェック1・2）を実行する | → 初-3 |
...
| 初-12 | AI | - | - | 「次回セッションからすべてのプロジェクトでフローが自動認識されます。次は自分の AI 開発プロジェクトのフォルダで Claude Code を起動し `/setup` を実行してください」と案内してここで終了する | → 初-13（別フォルダ・別セッションで実施） |

### プロジェクト設定（新しいプロジェクトに適用するたびに `/setup` で実行・冪等）

| ステップ | 担当 | 読み込み | 書き込み | すること | 次へ |
|---|---|---|---|---|---|
| 初-13 | 人間 | - | - | 対象プロジェクトのフォルダで Claude Code を起動し、`/setup` を実行する（PC 設定済みの場合はここから始まる） | → 初-14 |
| 初-14 | AI | `~/.claude/CLAUDE.md` → `~/.claude/ai-devguide-flow.md` | - | フロー命令書を読み込む | → 初-15 |
| 初-15 | AI | GitHub（ラベル一覧） | - | `eng:` ラベルが未設定か確認する。設定済みの場合は「フローが有効です」と案内して `/setup` を終了する。（現在地確認は `/setup` 外で行う：UserPromptSubmit フックが毎メッセージ Issue 一覧を `[issues:open]` として注入し、`ai-devguide-flow.md` のルーティングに従い `eng:wip` → `eng:review` → `eng:ready` → オープン Issue の順に自動報告する） | 未設定 → 初-16 ／ 設定済み → 終了（次メッセージからフックが自動ルーティング） |
...
→ 配布準備（初-0）でブートストラップを作り、PC 設定（初-1〜12）で gh 認証と `~/.claude/` へのフロー認識を整え、以降はどのプロジェクトでも `/setup` を実行するだけでプロジェクト設定（初-13〜20）が走る。各チェックは冪等（済みならスキップ）。
...
| 1-3 | AI | - | - | ①`gh auth status` で GitHub CLI の認証を確認（未インストールなら `operations/README.md §前提条件` に従い導入・認証）②リポジトリが未初期化（`eng:` ラベル未設定）なら `operations/README.md §B-1` を実施 | → 1-4 |
...
評価は3層に分け、**担い手が違う**：
- **spec verification（仕様形式の確認）** … Issueテンプレ・ラベル・ステップ記述の形式が揃っているか。**CLテスト（check-spec-coverage.sh 等）が機械的に確認**する。形式が揃っていなければ修正してから次の層へ。
- **implementation verification（実装の検証）** … 完了条件を実装が満たすか。**コードの正しさは GitHub の CI（自動テスト）が機械的に判定**する（AIの自己申告でも人間のコード読みでもない／重い機械仕事をGitHubに出しトークンも使わない）。満たさなければ下位工程（実装・構造・手法）に原因。
- **validation（向きが正しいか）** … この問題設定・手法で良かったか。**人間が結果の挙動を見て**判断する。間違っていれば上位工程（問題定義・手法）へ戻る、または中止。
...
| 5-2 | AI | 問題定義節／手法選定節／CI出力・エラー内容 | 評価節「方針の妥当性」「補助点検」「差し戻し案（No-goの場合）」 | 問題設定・手法は正しかったか（validation）＋完了条件外の品質の抜けを点検（4観点に固定しない）。**No-goの場合はCIエラー・コードを分析して差し戻し先（工程N）と理由を評価節に記録・人間へ提案する**（コードを読めるAIが診断し、コードを読めない人間の代わりに原因を特定する） | → 5-3 |
| 5-3 | 人間 | **CIの緑/赤 ＋ 動かした結果のデモ ＋ 評価節（差し戻し案含む）**（※コードは読まない） | 評価節「判定」 | 受け入れ（欲しかったものか）と方向（正しいか）を判断する。**No-goの場合はAIの差し戻し先提案を承認または修正する**（差し戻し先の診断はAIが行い、人間は方向性の判断のみ） | Go → 5-4／差し戻し承認 → AIが提案した工程へ／**validation不合格** → **工程1**か**工程2**へ／解く価値が無い → **中止** |
