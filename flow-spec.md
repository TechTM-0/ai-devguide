...
| 初-12 | AI | - | - | 「次回セッションからすべてのプロジェクトでフローが自動認識されます」と案内する | → 初-13（このまま続けてプロジェクト設定を実施） |
...
| 初-15 | AI | GitHub（ラベル一覧・Issue一覧） | - | `eng:` ラベルが未設定か確認する。設定済みの場合は `eng:wip` → `eng:review` → `eng:ready` → オープンIssue の順に確認し現在地を報告する。何もなければ工程1を開始する | 未設定 → 初-16 ／ 設定済みで進行中あり → 該当工程へ ／ 設定済みで何もなし → 工程1 |
